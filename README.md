...
web/                     сайт: без сборки, обычные модули JavaScript
  ...
  board/                   табло для проектора: board/?code=КОД (нужен вход)
  report/                  отчёт команды по ссылке: report/?t=…
  rating/                  рейтинг лиг, открыт без входа
  assets/js/config.js      адрес проекта Supabase, публичный ключ, регион базы
  assets/js/i18n.js        выбор языка, t() и tn()
  assets/js/i18n/          словари: en (основной), es, pt, ru
  assets/fonts/            Manrope и Unbounded: латиница и кириллица
...
- **Кто что видит.** Команда — только игры, где играет; ведущий — свои
  игры; администратор — все. Табло игры (и в игре, и на проекторе) — только
  её командам, ведущему и администраторам. Без входа открыты рейтинг лиг
  (только итоги, без почт) и отчёт команды по её ссылке — ссылку команда
  сама отдаёт напарникам.
- **Скорость.** База проекта — в Калифорнии (AWS us-west-1). Supabase
  запускает функцию рядом с игроком, и из Азии каждый запрос функции к базе
  пересекал бы океан. Поэтому сайт просит запускать функцию рядом с базой
  (`functionRegion` в `config.js`, параметр `forceFunctionRegion`): через
  океан идёт один запрос браузера, а не десяток запросов к базе. Проверка
  при развёртывании пишет регион базы и предупреждает, если он другой.
  Ещё: браузер запоминает разрешение CORS на сутки, токен входа проверяется
  на месте, запросы к базе идут параллельно, модули сайта грузятся разом
  (`npm run preload` обновляет список в HTML).

## Языки

Английский, испанский, португальский (Бразилия) и русский.

- Ведущий выбирает **язык игры** при создании (по умолчанию — язык, на
  котором у него сейчас сайт) и может сменить его в настройках игры.
- Каждый может выбрать **свой язык** в меню → «Настройки». «Автоматически» —
  в игре язык игры, в остальном сайте — язык браузера.
- Табло проектора всегда на языке игры: его читает зал.
- Деньги — доллары, записанные по-местному: $12,340, US$ 12.340, 12 340 $.
  CSV для португальского и русского — с «;» и десятичной запятой, как ждёт
  Excel.
- Новости команде и примечания журнала денег сервер пишет кодом с
  подробностями, а сайт собирает фразу на языке читателя.
- `node tests/i18n_check.mjs` проверяет, что все словари полные: те же
  ключи, подстановки и формы числа.
